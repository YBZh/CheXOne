...
if reader_id:
    ...
    reader_dict_residents = df_reader.set_index('unique_id')['resident'].to_dict()
    ...
    def save(survey, reader_id=reader_id, sample_id=None):
        ...
        # Extract only fields for this sample
        def _clean_label(lbl: str) -> str:
            # strip markdown bold and surrounding whitespace
            lbl = lbl.strip()
        ...
        sample_payload = {}
        # Keys are like: "edit_reader-{reader_id}_{sample_id}" etc.
        suffix = f"reader-{reader_id}_{sample_id}"
        for k, v in json_file.items():
            if isinstance(v, dict) and k.endswith(suffix):
                label = _clean_label(v.get("label", ""))
                value = v.get("value")
                # Remove trailing colon in label key for cleanliness
                key = label[:-1] if label.endswith(":") else label
                sample_payload[key] = value

        # Add blind mapping info and preferred source if chosen
        a_source, b_source = get_ab_sources(sample_id)
        sample_payload["A_source"] = a_source
        sample_payload["B_source"] = b_source
        preferred_label_key = "Which report demonstrates superior overall clinical quality?"
        preferred_value = sample_payload.get(preferred_label_key)
        if preferred_value in ("Report A", "Report B"):
            sample_payload["preferred"] = "A" if preferred_value == "Report A" else "B"
            sample_payload["preferred_source"] = a_source if preferred_value == "Report A" else b_source
        elif preferred_value == "Comparable (Equivalent Quality)":
            sample_payload["preferred"] = "Comparable (Equivalent Quality)"
    ...
    with pages:
        ...
        with right:
            ...
            a_source, b_source = get_ab_sources(sample)
            report_a_text = reader_dict_residents[sample] if a_source == 'resident' else reader_dict_reports[sample]
            report_b_text = reader_dict_residents[sample] if b_source == 'resident' else reader_dict_reports[sample]
            st.write('**Report A:**')
            st.write(report_a_text)
            # Display report
            st.write('**Report B:**')
            st.write(report_b_text)

            """#### 3. Choose Better Report:"""
            better_key = f"better_reader-{reader_id}_{sample}"
            better_selected = survey.selectbox(
                '**Which report demonstrates superior overall clinical quality?**',
                options=["", "Report A", "Report B", "Comparable (Equivalent Quality)"],
                index=0,
                id=better_key
            )

            # Only enable SAVE RESPONSES button if selection is made
            all_selected = (better_selected != "")
            def enable_next():
                if not all_selected:
